...
import { deepCloneTree, lastTraversalTree } from './utils/diffUtil';
...
var Tree = /*#__PURE__*/function (_React$Component) {
  ...
  function Tree() {
    ...
    _this.onNodeClick = function (e, treeNode) {
      var onClick = _this.props.onClick; // 右侧内容区域点击，触发 select/check 事件，不触发展开/收起事件
      // const { nativeEvent } = e;
      ...
      // 点击非子节点，切换展开收起状态
      // if (children !== undefined && selectable && !ctrlPick) {
    ...
    _this.onNodeSelect = function (e, treeNode) {
      ...
      var targetSelected = !selected; // 子节点且非 checkable 状态下才更新 selectedKey

      var selectable = children === undefined && !checkable; // Update selected keys
    ...
    _this.onNodeCheck = function (e, treeNode, checked) {
      ...
      if (checkStrictly) {
        ...
        eventObj.halfCheckedKeys = _halfCheckedKeys; // 最终选择，只选中叶子节点

        var finalCheckedKeys = [];
        ...
        checkedObj = [].concat(finalCheckedKeys);
    ...
    _this.getTreeNodeRequiredProps = function () {
      var _this$state7 = _this.state,
          ...
          hasTooltipKeys = _this$state7.hasTooltipKeys;
      return {
        expandedKeys: expandedKeys || [],
        selectedKeys: selectedKeys || [],
  ...
  _createClass(Tree, [{
    ...
    value: function render() {
      ...
      return /*#__PURE__*/React.createElement(TreeContext.Provider, {
        value: {
          ...
          checkboxTooltipText: this.props.checkboxTooltipText,
    ...
    value: function getDerivedStateFromProps(props, prevState) {
      ...
      function getHalfSelect(selecteds, _treeData) {
        var halfArr = [];
        var tooltipKeyArr = [];
        var cloneTree = deepCloneTree(_treeData);
        lastTraversalTree(function (item) {
          var children = item.children,
              key = item.key;
          var childrenRef = children && Array.isArray(children) ? children : [];
          var checkedChild = childrenRef.filter(function (curItem) {
            return curItem.childChecked === 1;
          });
          var noCheckedChild = childrenRef.filter(function (curItem) {
            return curItem.childChecked === 0;
          });
          var childLength = childrenRef.length;

          if (childLength > 0 && childrenRef.some(function (curItem) {
            return curItem.disabled;
          })) {
            tooltipKeyArr.push(item.key);
          } // 选项状态key处理


          if (childLength === 0 && selecteds.includes(key) || // 没有子项 且存在选中key
          childLength !== 0 && checkedChild.length === childLength // 有子项，看子项是否全部为选中
          ) {
            item.childChecked = 1; // item.text = '全选';
          } else if (childLength === 0 && !selecteds.includes(key) || // 没有子项，选中且不包含，
          childLength !== 0 && noCheckedChild.length === childLength // 有子项，看子项是否全部为不选中
          ) {
            item.childChecked = 0; // item.text = '未选中';
          } else {
            item.childChecked = 2; // item.text = '半选';

            halfArr.push(item.key);
          }
        ...
        return {
          halfArr: halfArr,
          tooltipKeyArr: tooltipKeyArr
        };
      }

      var _getHalfSelect = getHalfSelect(newState.checkedKeys || prevState.checkedKeys || [], props.treeData),
          halfArr = _getHalfSelect.halfArr,
          tooltipKeyArr = _getHalfSelect.tooltipKeyArr;

      newState.halfCheckedKeys = halfArr;
      newState.hasTooltipKeys = tooltipKeyArr; // ================= loadedKeys ==================
